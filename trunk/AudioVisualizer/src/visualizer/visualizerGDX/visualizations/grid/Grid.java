package visualizer.visualizerGDX.visualizations.grid;
...
import visualizer.visualizerGDX.AudioVisualizer;
import visualizer.visualizerGDX.Color;
import visualizer.visualizerGDX.ColorScheme;
...
public abstract class Grid extends Visualization
{
	...
	public byte[][] grid = new byte[numBars][AudioVisualizer.HEIGHT / cellSize];

	Sprite square;

	ColorScheme yellowish = new ColorScheme(new Color(1f, 1f, 0f, 1f),
			new Color(.7f, .7f, 0f, .7f));
	ColorScheme blueish = new ColorScheme(new Color(.3f, .5f, .9f, 1f),
			new Color(.1f, .3f, .9f, .7f));
	ColorScheme lighterblueish = new ColorScheme(new Color(.8f, .8f, 1f, 1f),
			new Color(.4f, .4f, 1f, .7f));
	ColorScheme redish = new ColorScheme(new Color(1f, .3f, .3f, 1f),
			new Color(1f, 0f, 0f, .7f));

	ColorScheme[] schemes =
	{ yellowish, blueish, lighterblueish, redish };
	...
	public Grid(SpriteBatch batch, float[] spectrum)
	{
		...
		square = new Sprite(new Texture(
				Gdx.files.internal("trunk/AudioVisualizer/res/square4.png")));
		...
		for (int j = 0; j < grid[0].length; j++)
		{
			for (int i = 0; i < grid.length; i++)
			{
				...
				} else if (grid[i][j] == 1)
				{
					if (i % 2 == 0)
					{
						square.setColor(color1.getR(), color1.getG(),
								color1.getB(), color1.getA());
					} else
					{
						square.setColor(color2.getR(), color2.getG(),
								color2.getB(), color1.getA());
					}
				} else if (grid[i][j] == 2)
				{
					if (i % 2 == 0)
					{
						square.setColor(color1.getR(), color1.getG(),
								color1.getB(), color2.getA());
					} else
					{
						square.setColor(color2.getR(), color2.getG(),
								color2.getB(), color2.getA());
					}
				}
	...
	/*
	 * public void transition() { float roc = 0.001f;
	 * 
	 * 
	 * 
	 * 
	 ...
	 * scheme.getColor2().setR(scheme.getColor2().getR() + roc); changed = true;
	 * } else if (newScheme.getColor2().getR() < scheme.getColor2().getR()) {
	 * scheme.getColor2().setR(scheme.getColor2().getR() - roc); changed = true;
	 * }
	 * 
	 * if (newScheme.getColor2().getG() > scheme.getColor2().getG()) {
	 * scheme.getColor2().setG(scheme.getColor2().getG() + roc); changed = true;
	 * } else if (newScheme.getColor2().getG() < scheme.getColor2().getG()) {
	 * scheme.getColor2().setG(scheme.getColor2().getG() - roc); changed = true;
	 * }
	 * 
	 * if (newScheme.getColor2().getB() > scheme.getColor2().getB()) {
	 * scheme.getColor2().setB(scheme.getColor2().getB() + roc); changed = true;
	 * } else if (newScheme.getColor2().getB() < scheme.getColor2().getB()) {
	 * scheme.getColor2().setB(scheme.getColor2().getB() - roc); changed = true;
	 * }
	 * 
	 * if (newScheme.getColor2().getA() > scheme.getColor2().getA()) {
	 * scheme.getColor2().setA(scheme.getColor2().getA() + roc); changed = true;
	 * } else if (newScheme.getColor2().getA() < scheme.getColor2().getA()) {
	 * scheme.getColor2().setA(scheme.getColor2().getA() - roc); changed = true;
	 * }
	 * 
	 * if (scheme.equals(newScheme)) { //scheme = newScheme.copy(); //newScheme
	 * = schemes[rand.nextInt(schemes.length)]; //scheme =
	 * schemes[rand.nextInt(schemes.length)]; newScheme =
	 * schemes[rand.nextInt(schemes.length)].copy(); System.out.println(true); }
	 * }
	 */
