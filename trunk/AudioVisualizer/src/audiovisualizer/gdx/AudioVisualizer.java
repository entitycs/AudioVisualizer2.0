 ...
 * @author Eric
 */
public class AudioVisualizer extends Game implements Runnable, TavVisualizationCustomizable
{
			...
			public void run()
			{
				...
				if (returnVal == JFileChooser.APPROVE_OPTION)
				{
					...
					try
					{
						...
						metaDataListener.metaDataUpdate (new TavMetaData("title: " + songName + "\n" + "artist: " + songArtist));
			...
			public void run()
			{
				...
				while (playing)
				{
							...
							samples.length)) > 0)
					{
						...
						// samples should really be converted to phases in
						// parent
						if (audioSpectrumListener != null)
							audioSpectrumListener.spectrumDataUpdate (0, 0,
									spectrum, samples);
	...
	public void setVisualization(int visualizationIndex)
	{
		this.currentVisualizationIndex = visualizationIndex;		
	}
